...
def optimize_pose(gaussians, view_cam, pipe, bg, pose_candidate, out_img_folder=""):
    ...
    for iter in range(num_iters_pose_opt):
        ...
        # Save images at the first and last iterations
        if iter == 0:
            torchvision.utils.save_image(
                image, os.path.join(out_img_folder, f"{view_cam.uid}_pose_before_opt.png")
            )
            torchvision.utils.save_image(
                image_gt, os.path.join(out_img_folder, f"{view_cam.uid}_gt.png")
            )
        if iter == num_iters_pose_opt - 1 or no_improve_counter >= patience:
            torchvision.utils.save_image(
                image, os.path.join(out_img_folder, f"{view_cam.uid}_pose_after_opt.png")
            )
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    sparse_recon_res_path = os.path.join(args.sparse_image_folder, f"sparse/0/sparse_{args.n_views}_ga.pkl")
    ...
    sparse_all_pair_point_clous = sparse_recon_res["point_clous"]
    ...
    base_num_iters_gaussian_opt = 200
        ...
        if level==2:
            for count in range(max(math.ceil(args.n_views/4), 1)):
                if count==0:
                    ...
                    pair_poses = naive_pair_poses @ np.linalg.inv(naive_pair_poses[0])
                    pair_point_cloud = sparse_all_pair_point_clous[start_index//4]
                    print(start_index, end_index, pair_poses)
            ...
            for count in range(max(math.ceil(args.n_views/(2**level)), 1)):
                ...
                if count==0:
                    ...
                    for ii, pose_tensor in enumerate(local_gaussian.P):
                        print(ii, pose_tensor)
                        scene_index = ii+ count + 2**(level-1)
                        if ii==0:
                            print(f"computing relative pose between view {scene_index} and view {scene_index - 2**(level-1)}")
                            next_pose_after_merge = sparse_extrinsics[scene_index] @ np.linalg.inv(sparse_extrinsics[scene_index - 2**(level-1)])
                            new_pose_tensor = get_tensor_from_camera(np.linalg.inv(next_pose_after_merge))
                        else:
                            pose_matrix = get_camera_from_tensor(pose_tensor).inverse().detach().cpu().numpy()
                            new_pose_matrix = next_pose_after_merge @ pose_matrix
                            new_pose_tensor = get_tensor_from_camera(np.linalg.inv(new_pose_matrix))
                        ...
                        if ii<4:
                            ...
                            if ii==0:
                                next_pose_after_merge = get_camera_from_tensor(optimal_pose).inverse().detach().cpu().numpy()
                    ...
                    new_c2w_global = torch.tensor(next_pose_after_merge).cuda()
                    gaussians_xyz = local_gaussian._xyz.clone()
                    gaussians_rot = local_gaussian._rotation.clone()
                    xyz_ones = torch.ones(gaussians_xyz.shape[0], 1).cuda().float()
                    xyz_homo = torch.cat((gaussians_xyz, xyz_ones), dim=1)
                    xyz_trans = (xyz_homo @ new_c2w_global.T)[:, :3]
                    quater = get_tensor_from_camera(new_c2w_global)
                    rot_trans = quadmultiply(quater[:4], gaussians_rot)
                    ...
                    if count*2 <= len(prev_level_list)-1:
                        ...
                        for ii, pose_tensor in enumerate(local_gaussian.P):
                            ...
                            if ii==0:
                                print(f"computing relative pose between view {global_scene_index} and view {global_scene_index - 2**(level-1)}")
                                next_pose_after_merge = sparse_extrinsics[global_scene_index] @ np.linalg.inv(sparse_extrinsics[global_scene_index - 2**(level-1)])
                                new_pose_tensor = get_tensor_from_camera(np.linalg.inv(next_pose_after_merge))
                            else:
                                pose_matrix = get_camera_from_tensor(pose_tensor).inverse().detach().cpu().numpy()
                                new_pose_matrix = next_pose_after_merge @ pose_matrix
