...
def readColmapCameras_wTrainFocal(cam_extrinsics, cam_intrinsics, images_folder, eval, model_path):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        poses.append(pose)
        #TODO: align with ours
        
        # if intr.model=="SIMPLE_PINHOLE":
        #     focal_length_x = train_fx #intr.params[0]
        #     FovY = focal2fov(focal_length_x, height)
        #     FovX = focal2fov(focal_length_x, width)
        # elif intr.model=="PINHOLE":
        focal_length_x = train_fx #intr.params[0]
        focal_length_y = train_fy #intr.params[1]
...
def readColmapSceneInfo(path, images, eval, args, opt, llffhold=2):
    ...
    if eval:
        # cam_infos = [c for idx, c in enumerate(cam_infos) if (idx+1) % llffhold != 0]
        # indices = np.linspace(0, len(cam_infos) - 1, 16, dtype=int)
        # print(indices)
        ...
        # train_poses = [c for idx, c in enumerate(sorted_poses) if idx in indices]
        # test_poses = [c for idx, c in enumerate(sorted_poses) if idx not in indices]
...
def readDust3RInfo(path, images, eval, scene_data: SceneData, args, opt, llffhold=2):
    ...
    # cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file)
    # cam_intrinsics = read_intrinsics_text(cameras_intrinsic_file)
    # run the first two scene data and get the 2 intrinsics and extrinscs
    # How to add/modify intrinsics and extrinsics?
