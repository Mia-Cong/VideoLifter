...
class Scene:
    ...
    def __init__(
        ...
        if not self.loaded_iter:
            # FIXME(Kevin): temporary disable save input.ply, because no ply_path to copy from
        ...
        # ic(scene_info.train_cameras)
        # ic(scene_info.train_poses)
        ...
        if self.loaded_iter:
            ...
            # self.gaussians.create_from_pcd(scene_info.point_cloud, scale_gaussian, self.cameras_extent)
            # breakpoint()
            ...
            # self.gaussians.init_RT_seq(self.test_cameras)
        # else:
    ...
    def addTrainViewPose_progressive(self, intrin, extrin, scale=1.0):
        ...
        ic(index)
        # focal_length_x = intrin[0,0]
        # width = int(intrin[0,2]*2)
        # height = int(intrin[1,2]*2)
    ...
    def addTrainView_progressive(self, intrin, extrin, pts, colors, scale=1.0):
        ic("new view in")
        # scale_gaussian = compute_scale_gaussian_by_project_pcd(pts, extrin, intrin)
        # self.gaussians.add_points_from_xyz_rgb(pts, colors, scale_gaussian)
        self.gaussians.add_points_from_xyz_rgb(pts, colors, None)
        
    def getTrainCameras_progressive(self, scale=1.0):
        # take only the the first progressive index of images
        ic("after adding new view")
        index = min(self.progressive_index, len(self.train_cameras[scale]))
        # ic(self.train_cameras[scale])
        # if index == self.progressive_index:
        #     # in true progressive instead of using the self.scene_data, should intinilize a new one with the correct images
        #     xyz, rgb = self.scene_data.get_pointcloud_with_index(index)
        #     self.gaussians.add_points_from_xyz_rgb(xyz, rgb)
        #     self.progressive_index += 1
        # else:
        #     raise Exception(
        #         "Sorry, all cameras are added, you should no longer call this mehod"
        #     )
        result = self.train_cameras[scale][:index]
        ic(self.gaussians._xyz.shape)
        return result

    # def addTrain_views(self, index):
    #     # TODO 1. update camera info
    # update pointcloud
