...
def resize_image(img, size=512, index=0):
    W1, H1 = img.size
    if size == 224:
    ...
    H2 = H // 16 * 16
    img = np.array(img)
    img = cv2.resize(img, (W2, H2), interpolation=cv2.INTER_NEAREST)
    img = PIL.Image.fromarray(img)
    ImgNorm = tvf.Compose([tvf.ToTensor(), tvf.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    img = dict(img=ImgNorm(img)[None], true_shape=np.int32(
            [img.size[::-1]]), idx=index, instance=str(index))
...
def init_minimum_spanning_tree(scene, focal_avg=False, known_focal=None, **kw):
    ...
    if known_focal is not None:
        ...
        print("before preset focal, the focal is",im_focals)
...
def save_colmap_cameras(ori_size, intrinsics, camera_file, write_flag='w'):
    with open(camera_file, write_flag) as f:
...
def save_colmap_images(poses, images_file, train_img_list, write_flag='w'):
    with open(images_file, write_flag) as f:
